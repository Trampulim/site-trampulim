/* ════════════════════════════════════════════════════════════
   META PIXEL — Grupo Trampulim (Novo) ID 866723092730172
   ════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  var langs = [['pt','PT','/'],['en','EN','/en/'],['es','ES','/es/']];
  ...
  // Exibe aviso de sucesso ou erro após o retorno do Netlify Forms
  // (o form redireciona para ?ok=1 ao enviar; anti-spam via honeypot + Akismet).
  var qs = new URLSearchParams(location.search);
  if (qs.get('ok') === '1') {
    var elOk = document.getElementById('form-ok');
    if (elOk) { elOk.style.display = 'block'; elOk.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
    history.replaceState(null, '', location.pathname);
  } else if (qs.get('erro') === '1') {
    var elErro = document.getElementById('form-erro');
    if (elErro) { elErro.style.display = 'block'; elErro.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
    history.replaceState(null, '', location.pathname);
  }
})();
